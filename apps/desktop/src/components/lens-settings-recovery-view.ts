      ...
      ${
        this.info
          ? html`<p role="alert">${this.info.message}</p>
              ...
              <div class="actions">
                ${this.confirming ? html`<button ?disabled=${this.busy} @click=${() => this.emit("restore")}>Restore Prompt Presets</button><button ?disabled=${this.busy} @click=${() => this.emit("cancel")}>Cancel</button>` : html`<button ?disabled=${this.busy} @click=${() => this.emit("open")}>Open Settings File</button><button ?disabled=${this.busy} @click=${() => this.emit("retry")}>Retry Loading</button>${this.info.can_restore_prompt_presets && this.info.digest ? html`<button ?disabled=${this.busy} @click=${() => this.emit("confirm")}>Restore Default Prompt Presets…</button>` : nothing}`}
              </div>`
