...
/** Sanitize ordinary Markdown before inserting DOM generated by the local math renderer. */
export function renderMarkdownFragment(markdown: string): DocumentFragment {
  const expressions: MathSpan[] = [];
  let prefix: string | undefined;
  let hasMath = false;
  const parser = new Marked({ async: false, breaks: false, gfm: true, pedantic: false });
  const lex = installMath(parser, (math) => {
    if (prefix === undefined) {
      hasMath = true;
      return "";
    }
    const index = expressions.push(math) - 1;
  ...
  if (!hasMath) return baseline;
  // Choose a deterministic marker absent from normalized user text. HTML entities
  // therefore cannot forge a marker. Only text nodes, never attributes, authorize insertion.
  const normalizedText = baseline.textContent ?? "";
  const occupiedMarkers = new Set<string>();
  for (const match of normalizedText.matchAll(/LENSMATH(\d+)_/gu)) {
    occupiedMarkers.add(match[1]);
  }
  let markerIndex = 0;
  while (occupiedMarkers.has(String(markerIndex))) markerIndex++;
  prefix = `LENSMATH${markerIndex}_`;
  const fragment = DOMPurify.sanitize(parser.parser(tokens), {
    ...SANITIZE_OPTIONS,
    RETURN_DOM_FRAGMENT: true,
  });
  ...
  const pattern = new RegExp(`${prefix}(\\d+)END`, "gu");
  let count = 0;
  let characters = 0;
  for (const node of nodes) {
    ...
    for (const match of node.data.matchAll(pattern)) {
      const math = expressions[Number(match[1])];
      if (!math) continue;
...
/** Compatibility for string-only consumers; DOM hosts should use the fragment API. */
export function renderMarkdown(markdown: string): string {
  const host = document.createElement("div");
  host.append(renderMarkdownFragment(markdown));
